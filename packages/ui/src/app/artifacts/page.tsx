...
export default function ArtifactsPage() {
  ...
  const emptyMessage =
    view === "archived"
      ? (t("artifacts.page.nothing_archived_artifacts_you_archive"))
      : (t("artifacts.page.no_artifacts_yet_reports_and"));
        ...
        <div className="max-w-5xl mx-auto">
          <div className="mb-6">
            <h1 className="text-2xl font-semibold text-fg mb-1">
              {t("artifacts.page.executive_artifacts")}
            </h1>
            <p className="text-sm text-fg-muted">
              {t("artifacts.page.every_markdown_deliverable_the_executive")}
            </p>
          </div>
          ...
          <div className="inline-flex items-center gap-1 mb-4 p-0.5 rounded-lg ring-1 ring-line bg-surface/40">
            {(["active", "archived"] as const).map((v) => (
              ...
              >
                {v === "active" ? t("artifacts.page.active") : t("artifacts.page.archived")}
              </button>
            ))}
          </div>
          ...
          {loading && (
            <div className="text-sm text-fg-muted">
              {t("artifacts.page.loading_artifacts")}
            </div>
          )}
          {error && (
            <div className="text-sm text-red-400 mb-4">
              {t("artifacts.page.error")}{error}
            </div>
          )}
            ...
            <>
              <div className="flex items-center gap-1 mb-5">
                <FilterButton
                  ...
                  label={t("artifacts.page.all")}
                  count={counts.all}
                />
                <FilterButton
                  active={filter === "draft"}
                  onClick={() => setFilter("draft")}
                  label={t("artifacts.page.drafts")}
                  count={counts.draft}
                />
                <FilterButton
                  active={filter === "workflow"}
                  onClick={() => setFilter("workflow")}
                  label={t("artifacts.page.workflows")}
              ...
              <div className="space-y-5">
                {groups.map((group) => {
                  ...
                  const displayGroupLabel = group.key === "drafts" ? (t("artifacts.page.drafts")) : group.label;
                  return (
                    <div key={group.key}>
                      <button
                        type="button"
